...
// useMiddlewares registers the middleware stack on e (outermost first), driven by
// the HTTP configuration.
func useMiddlewares(e *echo.Echo, httpCfg config.HTTP) {
	...
	e.Use(middleware.BodyLimit(httpCfg.BodyLimit))                                                  // reject oversized bodies
	e.Use(middleware.RateLimiter(middleware.NewRateLimiterMemoryStore(float64(httpCfg.RateLimit)))) // throttle per client IP
	e.Use(middleware.ContextTimeout(httpCfg.RequestTimeout))                                        // bound handler execution time
}
